...
const double EVCF[5][5] = {
    {1.7873015873015873e+00, -0.9359567901234568e+00, 0.1582175925925926e+00,
     -0.9755291005291005e-02, 0.1929012345679012e-03},
    {-0.4960317460317460e+00, 0.6057098765432098e+00, -0.1171296296296296e+00,
     0.7605820105820106e-02, -0.1543209876543210e-03},
    {0.1206349206349206e+00, -0.1632716049382716e+00, 0.4606481481481481e-01,
     -0.3505291005291005e-02, 0.7716049382716048e-04},
    {-0.1984126984126984e-01, 0.2779982363315696e-01, -0.8796296296296296e-02,
     0.8597883597883598e-03, -0.2204585537918871e-04},
    {0.1587301587301587e-02, -0.2259700176366843e-02, 0.7523148148148148e-03,
...
SP3File::SP3File(string inputFilePath, ios_base::openmode mode) {
  ...
  if (!fileStream) {
    cerr << "Error: unable to open SP3 file in constructor: " << pathFilename
         << " using mode: " << fileMode << endl;
...
int SP3File::readHeader() {
    ...
    if (recType[0] == '#' && recType[1] != '#') {
      ...
      if (formatVersion != 'a' && formatVersion != 'A') {
        cerr << "This program reads only SP3 files with format-mode: aP "
             << endl
             << "This file has format version: " << formatVersion << endl;
        return (-1);
      }
      modeFlag = inputRec[2];
      ...
      if (modeFlag != 'P' && modeFlag != 'p') {
        cerr << "This program reads only SP3 files with format-mode: aP "
             << endl
             << "This file has mode flag: " << modeFlag << endl;
        return (-1);
      }
      ...
      SP3EndTime.SetYMDHMS(year, month, day, hour, minute,
                           sec); // this will get added to later
  ...
  SP3EndTime =
      SP3EndTime + (double)(numberSP3epochs - 1) * SP3interval / 86400.0;

  if (numberGoodPRNs <= 0 || numberSP3svs != numberGoodPRNs) {
    cerr << "Error reading the PRNs from the header for SP3 file: " << endl
         << pathFilename << endl
...
int SP3File::getSVPosVel(DateTime tuser, unsigned short PRNid, double rvec[]) {
        ...
        for (k = 1; k <= 2; k++) {
          ...
          for (l = 1; l <= 5; l++) {
            ...
            for (m = 5; m >= 2; m--) {
              intrpCoeff[l][k][j][i] =
                  intrpCoeff[l][k][j][i] +
                  EVCF[m - 1][l - 1] * (inputValues[jmid + m - 1][j][i] +
                                        inputValues[jmid - m + 1][j][i]);
              // dout << " 1st iloop l,k,j,i,m: " << l << " " << k << " " << j
              // << " " << i
              //      << " " << m << endl;
            }
            intrpCoeff[l][k][j][i] =
  ...
  for (m = mbeg; m <= mend; m++) {
    ...
    if (m <= numberSVparams)
      ...
      rvec[muse - 1] =
          (intrpCoeff[1][2][m][jsv] + sum1) - (intrpCoeff[1][1][m][jsv] + sum2);
  }
